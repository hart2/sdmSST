...
# OISST product is a global 1/4 degree gridded dataset of Advanced Very High 
# Resolution Radiometer (AVHRR) derived SSTs at a daily resolution, starting on 
# 1 September 1981. The source of the data is currently the NOAA NCDC.
...
# Make sure that they are currently available on an ERDDAP server. The location 
# of the NOAA OISST data has changed in the past so it should not be assumed 
# that the current location will exist in perpetuity.
...
# This function downloads and prepares data based on user provided start and end 
# dates AND user specified lat lon limits
...
# Because we can’t download all of the data in one request, and we can’t download
# the data one year at a time, we will need to make requests for multiple batches 
# of data. To accomplish this we will create a dataframe of start and end dates
# that will allow us to automate the entire download while meeting the aforementioned
# criteria.
...
# If the below chunk of code fails or times out, simply re-run it until all of 
...
# It should only take a few seconds to load one month of data depending on the 
# size of the lon/lat extent chosen
